...
class HiFTGenerator(nn.Module):
    ...
    def remove_weight_norm(self):
        ...
        def safe_remove_weight_norm(layer):
            try:
        ...
        safe_remove_weight_norm(self.conv_pre)
        safe_remove_weight_norm(self.conv_post)
        ...
        for l in self.source_downs:
            safe_remove_weight_norm(l)
        for resblock in self.source_resblocks:
            for conv in resblock.convs1:
                safe_remove_weight_norm(conv)
            for conv in resblock.convs2:
                safe_remove_weight_norm(conv)

        safe_remove_weight_norm(self.m_source.l_linear)  # 여기만 처리 가능
        for layer in self.f0_predictor.condnet:
