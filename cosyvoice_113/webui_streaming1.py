...
def generate_tts(current_text, seed):
    current_text = current_text.strip("\n") # 기본 strip()(인자 없음)을 쓰면, 양끝의 모든 공백류(스페이스, 탭, 개행 등)를 삭제 방지
    debug_log = ""
...
def launch_demo():
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            audio_output = gr.Audio(
                ...
                autoplay=True,
                streaming=True, # 이 컴포넌트가 조각(chunk) 단위로 점진적으로 값이 들어오는 걸 전제로 동작, 
                #백엔드 함수가 제너레이터로 동작하면서 yield (sr, chunk)를 반복하면, Gradio가 그 조각들을 이어붙여 즉시 재생. 
                # 즉, 전체가 끝날 때까지 기다리지 않고 실시간 재생
                type="numpy",
                format="wav"
            )
        ...
        # 직접적으로 debounce 로직 없이 기본 함수만 연결 , 텍스트박스의 값이 바뀔 때마다 이벤트
        textbox.change(
            fn=generate_tts,
            inputs=[textbox, seed],
    ...
    demo.queue(max_size=10, default_concurrency_limit=1) # 그라디오 서버의 큐(대기열)와 바인딩 설정
    # max_size=10: 동시에 대기할 수 있는 요청 갯수 상한
    # default_concurrency_limit=1: 동시에 딱 1개 작업만 실행
