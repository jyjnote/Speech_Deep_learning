...
def padding(data, use_spk_embedding, mode='train', gan=False, dpo=False):
    """ Padding the data into training data
        ...
        Returns:
            Iterable[Dict]
    """
    chunk_multiple = 4  # 👈 flow decoder에서 요구하는 최소 chunk 배수
    for sample in data:
        ...
        # ✅ [핵심] x (speech_feat) 와 mu (speech_token)의 time length를 맞추기
        min_len = min(speech_feat.shape[1], speech_token.shape[1])

        # ✅ chunk_multiple 단위로 자르기
        min_len = (min_len // chunk_multiple) * chunk_multiple

        # ✨ 디버깅
        #print(f"[DEBUG] Before alignment: speech_feat={speech_feat.shape}, speech_token={speech_token.shape}")
        #print(f"[DEBUG] Aligning time length to min_len={min_len} (multiple of {chunk_multiple})")

        # ✅ 실제 자르기
        speech_feat = speech_feat[:, :min_len]
        speech_token = speech_token[:, :min_len]
        speech_feat_len = torch.full((speech_feat.size(0),), min_len, dtype=torch.int32)
        speech_token_len = torch.full((speech_token.size(0),), min_len, dtype=torch.int32)

        # ✨ 디버깅
        #print(f"[DEBUG] After  alignment: speech_feat={speech_feat.shape}, speech_token={speech_token.shape}")
        ...
        batch["embedding"] = batch["spk_embedding"] if use_spk_embedding else batch["utt_embedding"]
